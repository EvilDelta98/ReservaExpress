...
<head>
  <?php
    ...
    if($cliente->getPerfilId()=="0"){
      $perf= "Prueba";
    }
    if($cliente->getPerfilId()=="1"){
      $perf= "Operador";
    }
    if($cliente->getPerfilId()=="2"){
      $perf= "Administrador";
    }
  ...
  <center> 
    ...
    <form class="form-label" id="formModificarU" name="formModificarU" method="POST" action="operador/update">
      <div class="col-sm-2">
        ...
        <input value="<?= $cliente->getApellido() ?>" maxlength="255" required pattern="[a-zA-ZÀ-ÿ\s]{5,45}" type="text" name="datoApellido" id="datoApellido">
      </div>
      <div class="col-sm-2">
        <label for="">Nombre</label>
        <input value="<?= $cliente->getNombre() ?>" minlength="3" maxlength="45" required pattern="[a-zA-ZÀ-ÿ\s]{5,45}" type="text" name="datoNombre" id="datoNombre">
      </div>
      <div class=" col-sm-2">
